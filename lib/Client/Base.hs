module Client.Base (open, send, Client.Base.close, receive, close_) where
...
import Utils.MqttException ( MqttException(DisconnectException) )
...
import Utils.IO ( mkPacketIdCounter )
...
--- *** Close client *** ---
close :: ConnAction (IO ())
close = close_ <$> getConn

close_ :: Connection -> IO ()
close_ conn = do
